...
class UserRepo extends CoreRepo
{
    ...
    public function create(array $data): ?User
    {
        ...
        if (array_key_exists('direction', $data) && $data['direction']['id'] === 'new') {
            $direction = $directionRepo->create([
                'name' => $data['direction']['name'],
            ]);
            $data['direction_id'] = $direction->getKey();
        }
        if (!$role = Role::findByName($data['role'])) {
